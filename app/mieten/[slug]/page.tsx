...
export async function generateMetadata({ params }: PageProps) {
  ...
  const tradeArr = page.trade as any
  const cityArr = page.city as any
  const tradeName = tradeArr?.[0]?.name || 'Handwerker'
  const cityName = cityArr?.[0]?.name || 'Ihrer Stadt'

  return {
    title: `Diese Seite mieten — ${tradeName} in ${cityName}`,
  }
}
          ...
          <div className="space-y-4">
            {[
              ...
              },
              {
                ...
                a: 'Sofort. Die Seite ist bereits live und rankt. Ihre Daten sind innerhalb von 24h eingebunden.'
              },
              {
                q: 'Kann ich kündigen?',
                a: 'Ja, jederzeit mit 30 Tagen Frist. Keine Bindung, keine Strafe.'
              },
              {
